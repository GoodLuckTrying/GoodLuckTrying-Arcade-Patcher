...
set "PS1_SCRIPT=Get-CRC32-gngt.ps1"
...
set "filename=mm01.11e" & set "expected_crc=ecfccf07" & set /a total_files+=1 & if not exist "%ROM_DIR%\!filename!" (echo [MISSING] !filename! & set /a missing_count+=1) else (for /f "delims=" %%h in ('powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0%PS1_SCRIPT%" "%ROM_DIR%\!filename!" 2^>nul ^| findstr /r /c:"^[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]$"') do set "actual_crc=%%h" & if "!actual_crc!"=="!expected_crc!" (echo [PASS] !filename! - CRC32: !actual_crc! & set /a passed_count+=1) else (echo [FAIL] !filename! & echo        Expected: !expected_crc! & echo        Actual:   !actual_crc! & set /a failed_count+=1))
