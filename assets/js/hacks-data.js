window.HACKS_DATA = {
  "ghouls-artoria-v10": {
    "id": "ghouls-artoria-v10",
    "title": "Ghouls'n Ghosts: Knight/Maiden Artoria Edition v1.0",
    "previewsFolder": "assets/previews/ghouls-artoria-v10",
    "previews": [
      "1. Nightgown.png",
      "2. Knight Artoria 1.png",
      "2. Knight Artoria 2.png",
      "2. Knight Artoria 3.png",
      "2. Maid Artoria 1.png",
      "2. Maid Artoria 2.png",
      "3. Gold Armor (+ Knife Special) 1.png",
      "3. Gold Armor (+ Knife Special) 2.png",
      "4. Gold Armor (+ Axe Special).png",
      "5. Gold Armor (+ Sword Special) 1.png",
      "5. Gold Armor (+ Sword Special) 2.png",
      "5. Gold Armor (+ Sword Special) 3.png",
      "5. Gold Armor (+ Sword Special) 4.png",
      "6. Transformation #1.png",
      "Axe Special.gif",
      "Knight End Screen 1.png",
      "Knight End Screen 2.png",
      "Maiden End Screen 1.png",
      "Maiden End Screen 2.png",
      "Psycho Cannon.png",
      "Sword Special.gif"
    ],
    "trailerUrl": "https://youtu.be/kNsi4sraUQc",
    "downloadUrl": "Ghouls'n Ghosts - Artoria Edition v1.0 by GoodLuckTrying/Ghouls'n Ghosts - Artoria Edition v1.0 by GoodLuckTrying.zip",
    "boxartLinks": [
      {
        "label": "Ghouls'n Ghosts: Knight/Maiden Artoria Edition",
        "url": "https://www.deviantart.com/xxcaesarxx/art/Ghouls-n-Ghosts-Knight-Maiden-Artoria-Edition-1373422334"
      },
      {
        "label": "Daimakaimura: Knight/Maiden Artoria Edition",
        "url": "https://www.deviantart.com/xxcaesarxx/art/Daimakaimura-Knight-Maiden-Artoria-Edition-1373423892"
      }
    ],
    "icon": "assets/icons/GhoulsArtoria.png"
  },
  "gng-enhanced-v10": {
    "id": "gng-enhanced-v10",
    "title": "Ghosts'n Goblins Enhanced v1.0",
    "previewsFolder": "assets/previews/gng-enhanced-v10",
    "previews": [
      "Ghosts'n Goblins - Enhanced Edition.gif",
      "Ghosts'n Goblins - Enhanced Edition.png",
      "Makaimura - Enhanced Edition.png"
    ],
    "trailerUrl": "https://youtu.be/1MqTjmTokAo",
    "downloadUrl": "Ghosts'n Goblins Enhanced v1.0 by GoodLuckTrying/Ghosts'n Goblins Enhanced v1.0 by GoodLuckTrying.zip",
    "icon": "assets/icons/GoblinsArthur.png"
  },
  "gng-artoria-v12": {
    "id": "gng-artoria-v12",
    "title": "Ghosts'n Goblins: Knight/Maiden Artoria Edition v1.2",
    "previewsFolder": "assets/previews/gng-artoria-v12",
    "previews": [
      "Knight Icon.png",
      "knight01.png",
      "knight02.png",
      "Maiden Icon.png",
      "maiden01.png",
      "maiden02.png"
    ],
    "trailerUrl": "https://youtu.be/NHejslEFf_g",
    "downloadUrl": "Ghosts'n Goblins - Artoria Edition v1.2 by GoodLuckTrying/Ghosts'n Goblins - Artoria Edition v1.2 by GoodLuckTrying.zip",
    "boxartLinks": [
      {
        "label": "Knight Artoria Edition",
        "url": "https://www.deviantart.com/xxcaesarxx/art/Ghosts-n-Goblins-Makaimura-Knight-Artoria-Edition-1312785645"
      },
      {
        "label": "Maiden Artoria Edition",
        "url": "https://www.deviantart.com/xxcaesarxx/art/Ghosts-n-Goblins-Makaimura-Maiden-Artoria-Edition-1312786516"
      }
    ],
    "icon": "assets/icons/GoblinsArtoria.png"
  }
};
